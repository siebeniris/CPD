...
def get_cpd_df(file, aspect):
    ...
    df = df[df[aspect] == True]
    cpd_df = df.groupby(['date', 'uid'])['sentiment'].mean().reset_index()
...
def get_info_list(df):
    ...
    return list(zip(dates, uids, lemmas, sentences))
...
def write_out_to_csv(sentences, outputfile):
    ...
    with open(outputfile, 'w', newline='') as csvfile:
        ...
        csvwriter.writerow(['date', 'uid', 'lemma', 'sentence', 'typical?(renovation) yes or no'])
...
if __name__ == '__main__':
    ...
    output_dir = os.path.join(root_dir, "data", "selected_reviews")
            ...
            for aspect in aspects:
                ...
                if not os.path.exists(exceptionfile):
                    if not os.path.exists(jsonfile) or not os.path.exists(csvfile):
                        cpd_df, df = get_cpd_df(filepath, aspect=aspect)
                        # emas plot.
                        ...
                        try:
                            ...
                            if any(sentences.values()):
                                ...
                                with open(exceptionfile, 'a+') as file:
                                    file.write(aspect + "=> empty sentences")
