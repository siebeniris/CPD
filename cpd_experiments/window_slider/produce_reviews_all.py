...
def get_cpd_df(inputfile: str, outputfile: str) -> pd.DataFrame:
    ...
    if not os.path.exists(outputfile):
        ...
        df['date'] = df['date'].swifter.apply(lambda x: pd.to_datetime(x, errors='coerce')).dropna(). \
            dt.strftime('%Y-%m-%d')
        ...
        df['renovation'] = df.swifter.apply(lambda x: sum([y in x.lemma.lower().split() for y in renovations]) > 0,
                                            axis=1)
        df['other'] = df.swifter.apply(lambda x: sum([y in x.lemma.lower().split() for y in aspects]) > 0, axis=1)
        # consider it long sentence if length of words more than 3.
        df['long'] = df.swifter.apply(lambda x: len(x.sentence.split()) > 3, axis=1)
        print(df['sentence'].to_list()[:3])
        print(df['renovation'].to_list()[:3])
        print(df['other'].to_list()[:3])
        print(df['long'].to_list()[:3])
    ...
    scores = []
    # sort dates here.
    for date in sorted(list(set(df.date.to_list()))):
        score = 0
...
def get_dataset(df, trends, neg_fac_reno= 10,  neg_fac=10):
    """
    Get Dataset
    ...
    :return:
    """
    reno_df = df[(df['renovation'] == True) & (df['other'] == True) & (df['long'] == True)]
    other_df = df[(df['renovation'] == False) & (df['other'] == True) & (df['long'] == True)]
...
if __name__ == '__main__':
    ...
    neg_fac = 10
    count = 0
    path = ['488#63f14bfe-aac7-4e87-91f1-48233ea92330']
            ...
            if not os.path.exists(outputfile):
                ...
                if os.path.isfile(filepath):
                    ...
                    if os.path.exists(aspect_file):
                        ...
                        if len(trends) > 0:
                            sentences, trends = get_dataset(df_new, trends, neg_fac_reno=-1,  neg_fac=neg_fac)
